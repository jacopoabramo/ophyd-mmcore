...
from ophyd_mmcore import PropName
from ophyd_mmcore._stage import MMXYStage, MMZStage
...
async def test_z_locate_returns_current_position(worker_z: MMCoreWorker) -> None:
    """locate() returns the current Z position as setpoint and readback."""
    stage = ZStage("ZDrive", worker_z, name="z")
...
async def test_z_set_calls_set_position(
    worker_z: MMCoreWorker, core_z: CMMCorePlus
) -> None:
...
async def test_z_set_updates_locate_after_move(worker_z: MMCoreWorker) -> None:
...
async def test_z_set_emits_watcher_updates(
    worker_z: MMCoreWorker, core_z: CMMCorePlus
) -> None:
...
async def test_z_stop_calls_core_stop(
    worker_z: MMCoreWorker, core_z: CMMCorePlus
) -> None:
...
async def test_z_stop_marks_move_as_failed(
    worker_z: MMCoreWorker, core_z: CMMCorePlus
) -> None:
    """stop(success=False) causes the in-progress set() to raise."""
    # Keep stage busy long enough for stop() to be called
    call_count = 0

    async def _slow_busy(label: str) -> bool:
        nonlocal call_count
        call_count += 1
        return call_count < 10  # busy for first 9 polls
...
async def test_xy_locate_returns_current_position(worker_xy: MMCoreWorker) -> None:
    """locate() returns the current (x, y) position."""
    stage = XYStage("XYDrive", worker_xy, name="xy")
    await stage.connect(mock=False)
...
async def test_xy_set_calls_set_xy_position(
...
async def test_xy_set_updates_locate_after_move(worker_xy: MMCoreWorker) -> None:
...
async def test_xy_set_emits_watcher_updates(
    worker_xy: MMCoreWorker, core_xy: CMMCorePlus
) -> None:
...
async def test_xy_stop_calls_core_stop(
    worker_xy: MMCoreWorker, core_xy: CMMCorePlus
) -> None:
    """stop() calls core.stop with the device label."""
    stage = XYStage("XYDrive", worker_xy, name="xy")
...
def test_z_mv_in_plan(
    RE: RunEngine, worker_z: MMCoreWorker, core_z: CMMCorePlus
) -> None:
...
def test_xy_mv_in_plan(
    RE: RunEngine, worker_xy: MMCoreWorker, core_xy: CMMCorePlus
) -> None:
...
def test_z_locate_in_plan(RE: RunEngine, worker_z: MMCoreWorker) -> None:
